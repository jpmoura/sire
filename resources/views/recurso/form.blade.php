    ...
    <div class="input-group">
        ...
        <select class="form-control" name="tipo" required data-toggle="tooltip" data-placement="right"
            ...
            @foreach($tipos as $tipo)
                <option value="{{ $tipo->id }}" @if(Route::is('detailsAsset') && $recurso->tipo_recurso_id == $tipo->id) selected @endif>{!! $tipo->nome !!}</option>
            @endforeach
        </select>
    </div>
    @if ($errors->has('tipo'))
...
<div class="form-group {{ $errors->has('nome') ? 'has-error' : '' }}">
    <div class="input-group">
        ...
        <input type="text" class="form-control" placeholder="Nome. Máximo de 50 caracteres." autofocus name="nome"
               ...
               value="@if($errors->has('nome')){{ old('nome') }}@elseif(Route::is('detailsAsset')){{ $recurso->nome }}@endif">
    </div>
    @if ($errors->has('nome'))
        <span class="help-block">
...
<div class="form-group {{ $errors->has('descricao') ? 'has-error' : '' }}">
    <div class="input-group">
        ...
        <textarea class="form-control" name="descricao" rows="4" cols="40" placeholder="Descrição. Máximo de 100 caracteres."
                  ...
                  title="Descrição. Máximo de 100 caracteres.">@if($errors->has('descricao')){!! old('descricao') !!}@elseif(Route::is('detailsAsset')){!! $recurso->descricao !!}@endif</textarea>
    </div>
...
<div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
    <div class="input-group">
        ...
        <select class="form-control" name="status" required data-toggle="tooltip" data-placement="right" title="Status do recurso.">
            <option value="">Selecione</option>
            <option value="1" @if(($errors->has('status') && old('status') == 1) || (Route::is('detailsAsset') ) && $recurso->status == 1) selected @endif>Ativo</option>
            <option value="0" @if(($errors->has('status') && old('status') == 0) || (Route::is('detailsAsset') ) && $recurso->status == 0) selected @endif>Inativo</option>
        </select>
    </div>
